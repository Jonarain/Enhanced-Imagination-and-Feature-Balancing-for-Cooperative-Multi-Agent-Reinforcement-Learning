...
class DMAQ_SI_Weight(nn.Module):
        ...
        for i in range(self.num_kernel):  # multi-head attention
            if getattr(args, "adv_hypernet_layers", 1) == 1:
                ...
                self.key_extractors.append(
                    nn.Sequential(nn.Linear(self.state_dim, adv_hypernet_embed), nn.ReLU(), nn.Linear(adv_hypernet_embed, 1)))  # key
                self.agents_extractors.append(
                    nn.Sequential(nn.Linear(self.state_dim, adv_hypernet_embed), nn.ReLU(), nn.Linear(adv_hypernet_embed,
                                                                                                      self.n_agents)))  # agent
                self.action_extractors.append(
                    nn.Sequential(
                        nn.Linear(self.state_action_dim, adv_hypernet_embed), nn.ReLU(), nn.Linear(adv_hypernet_embed,
                                                                                                   self.n_agents)))  # action
            elif getattr(args, "adv_hypernet_layers", 1) == 3:
                adv_hypernet_embed2 = self.args.adv_hypernet_embed2
                self.key_extractors.append(
                    nn.Sequential(
                        nn.Linear(self.state_dim, adv_hypernet_embed),
                        nn.ReLU(),
                        nn.Linear(adv_hypernet_embed, adv_hypernet_embed2),
                        nn.ReLU(),
                        nn.Linear(adv_hypernet_embed2, 1)))  # key
                self.agents_extractors.append(
                    nn.Sequential(
                        nn.Linear(self.state_dim, adv_hypernet_embed),
                        nn.ReLU(),
                        nn.Linear(adv_hypernet_embed, adv_hypernet_embed2),
                        nn.ReLU(),
                        nn.Linear(adv_hypernet_embed2, self.n_agents)))  # agent
                self.action_extractors.append(
                    nn.Sequential(
                        ...
                        nn.Linear(adv_hypernet_embed, adv_hypernet_embed2),
                        nn.ReLU(),
                        nn.Linear(adv_hypernet_embed2, self.n_agents)))  # action
            else:
    ...
    def forward(self, states, actions):
        ...
        for curr_head_key, curr_head_agents, curr_head_action in zip(all_head_key, all_head_agents, all_head_action):
            ...
            weights = x_key * x_agents * x_action # [batch*step, 1] * [batch*step, n_agents] *[batch*step, n_agents]
