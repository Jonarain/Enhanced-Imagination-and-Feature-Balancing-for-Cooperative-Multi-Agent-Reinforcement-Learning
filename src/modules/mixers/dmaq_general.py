...
class DMAQer(nn.Module):
    ...
    def __init__(self, args, state_dim=None):
        ...
        if getattr(args, "adv_hypernet_layers", 1) == 3:
            hypernet_embed2 = self.args.hypernet_embed2
            self.hyper_w_final = nn.Sequential(nn.Linear(self.state_dim, hypernet_embed),
                                               nn.ReLU(),
                                               nn.Linear(hypernet_embed, hypernet_embed2),
                                               nn.ReLU(),
                                               nn.Linear(hypernet_embed2, self.n_agents))
            ...
            self.V = nn.Sequential(nn.Linear(self.state_dim, hypernet_embed),
                                   nn.ReLU(),
                                   nn.Linear(hypernet_embed, self.n_agents))
        elif getattr(args, "adv_hypernet_layers", 1) == 1:
            self.hyper_w_final = nn.Sequential(nn.Linear(self.state_dim, self.n_agents), )
            self.V = nn.Sequential(nn.Linear(self.state_dim, self.n_agents))
        # self.hyper_w_final = nn.Sequential(nn.Linear(self.state_dim, hypernet_embed), nn.ReLU(), nn.Linear(hypernet_embed, self.n_agents))
        # self.V = nn.Sequential(nn.Linear(self.state_dim, hypernet_embed), nn.ReLU(), nn.Linear(hypernet_embed, self.n_agents))

        self.si_weight = DMAQ_SI_Weight(args, state_dim=state_dim)
    ...
    def forward(self, agent_qs, states, actions=None, max_q_i=None, is_v=False):
        ...
        if self.args.weighted_head: # default: True
    ...
    def calc_v_test(self, agent_qs):
        agent_qs = agent_qs.view(-1, self.n_agents)
        v_tot = th.sum(agent_qs, dim=-1)
        return v_tot

    def calc_adv_test(self, agent_qs, states, actions, max_q_i):
        states = states.reshape(-1, self.state_dim)
        actions = actions.reshape(-1, self.action_dim)
        agent_qs = agent_qs.view(-1, self.n_agents)
        max_q_i = max_q_i.view(-1, self.n_agents)
        ...
        adv_tot = th.sum(adv_q * adv_w_final, dim=1)
        return adv_tot, th.sum(adv_q, dim=1)

    def calc_test(self, agent_qs, states, actions=None, max_q_i=None, is_v=False):
        if is_v:
            v_tot = self.calc_v_test(agent_qs)
            return v_tot, None
        else:
            adv_tot, adv_sum = self.calc_adv_test(agent_qs, states, actions, max_q_i)
            return adv_tot, adv_sum
    def forward_test(self, agent_qs, states, actions=None, max_q_i=None, is_v=False):
        bs = agent_qs.size(0)
        states = states.reshape(-1, self.state_dim)
        agent_qs = agent_qs.view(-1, self.n_agents)
        ...
        y, y_sum = self.calc_test(agent_qs, states, actions=actions, max_q_i=max_q_i, is_v=is_v)
