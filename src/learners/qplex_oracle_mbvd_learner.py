...
class Qplex_OracleMBVDLearner:
    ...
    def __init__(self, mac, scheme, logger, args):
        ...
        if args.mixer is not None:
            if args.mixer == "dmaq":
                self.mixer = DMAQer(args, state_dim=hyper_input_dim)
            else:
    ...
    def model_train(self, batch, input):
        ...
        for k in range(self.args.k_step):  # k_step=1
            prior_mu, prior_logvar, next_prior_sample_state = self.model.prior_forward(prior_sample_state,
                                                                                       past_onehot_action[:,
                                                                                       k: length + k])

            # L_KL prior all + posterior all
            # KL(p|N(0,1))
    ...
    def sub_train(self, batch: EpisodeBatch, t_env: int, episode_num: int, mac, mixer, optimiser, params,
        ...
        posterior_mu, posterior_logvar, sample_state, obs_mse_loss, kld_loss, avail_actions_loss, next_prior_sample_state, rew_loss, prior_kld = self.model_train(
        ...
        for h in range(current_rollout_depth):
            with th.no_grad():
                ...
                zero_mask = th.all(i_avail_actions == 0, dim=-1)  # 32, 201, 8
                all_zero_ava_prev += zero_mask.sum()

                add_ten = th.zeros_like(i_avail_actions, device=self.args.device, dtype=th.float32)  # 32, 201, 8, 6
        ...
        if self.args.use_combiner and self.args.combiner_regular:
            ...
            w_loss = w_reg[mask == 1].mean() * self.args.reg_beta
            loss += w_loss
        ...
        if t_env - self.log_stats_t >= self.args.learner_log_interval:
            ...
            if self.args.use_combiner and self.args.combiner_regular:
                self.logger.log_stat("w_loss", w_loss.item(), t_env)
    ...
    def load_models(self, path):
        ...
        if self.args.use_combiner:
            self.combiner.load_state_dict(
                th.load("{}/combiner.th".format(path), map_location=lambda storage, loc: storage))
    ...
    def trainable_params_print(self):
        ...
        if self.args.use_combiner:
            self.logger.console_logger.info(
                f"combiner params:{sum(param.numel() for param in self.combiner.parameters())}")
        self.logger.console_logger.info(
