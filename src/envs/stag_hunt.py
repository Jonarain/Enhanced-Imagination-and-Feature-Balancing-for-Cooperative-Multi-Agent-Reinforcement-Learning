# Ref: https://github.com/wendelinboehmer/dcg/blob/master/src/envs/stag_hunt.py
'''
...
class StagHunt(MultiAgentEnv):
    ...
    def __init__(self, batch_size=None, **kwargs):
        ...
        self.if_save_replay = args.env_save_replay
        if self.if_save_replay:
            self.grid_mem = []
            self.steps_mem = []
            self.agents_xy_mem = []
            self.agents_not_frozen_mem = []
        ...
        self.steps = 0
        self.sum_rewards = 0
        self.episode_success_capture = 0.               # the number of successfully catch during one episode
        self.episode_capture_attempt = 0.
        self.reset()
    ...
    def reset(self):
        ...
        self.steps = 0
        self.sum_rewards = 0
        self.episode_success_capture = 0.
        self.episode_capture_attempt = 0.

        # Clear the grid
        self.grid.fill(0.0)

        #save all 0 grid as a break of episode for replay
        if self.if_save_replay:
            self.grid_mem.append(self.grid.copy())
            self.steps_mem.append(-1)
            self.agents_xy_mem.append(self.agents.copy())
            self.agents_not_frozen_mem.append(self.agents_not_frozen.copy())
        ...
        if self.if_save_replay:
            self.grid_mem.append(self.grid.copy())
            self.steps_mem.append(self.steps)
            self.agents_xy_mem.append(self.agents.copy())
            self.agents_not_frozen_mem.append(self.agents_not_frozen.copy())
    ...
    def step(self, actions):
        # actions shape:torch.Size([8]), batch mode:False
            ...
            for a in np.random.permutation(self.n_agents):
                ...
                if self.agents_not_frozen[a, b] > 0:
                    if actions[a, b] == self.action_labels['catch']:
                        self.episode_capture_attempt += 1
        ...
        for b in range(self.batch_size):
            for p in np.random.permutation(self.n_prey):
                if self.prey_alive[p, b] > 0:
                    ...
                    if captured:
                        self.episode_success_capture += n_catching_agents
                        # kill prey
        ...
        info["episodic_success_capture"] = self.episode_success_capture
        info["episodic_capture_attempt"] = self.episode_capture_attempt
        info["episodic_success_capture_ratio"] = self.episode_success_capture/self.episode_capture_attempt if self.episode_capture_attempt!=0 else 0.
        info["episodic_failed_capture"] = self.episode_capture_attempt-self.episode_success_capture
    ...
    # --------- RENDER METHODS -----------------------------------------------------------------------------------------
    def return_grid(self, batch=0, grid=None):
        if grid is None:
            grid = self.grid
    ...
    def save_replay(self):
        ...
        fig, ax = plt.subplots()
        #(688, 1, 10, 10, 3)
        init = self.return_grid(grid=self.grid_mem[0])
        extent = [0, len(init[0]), len(init), 0]
        x_ticks = np.arange(extent[0], extent[1], 1)
        y_ticks = np.arange(extent[3], extent[2], 1)
        plt.xticks(x_ticks)
        plt.yticks(y_ticks)
        plt.grid(True)

        for g, s, a_xy, a_nfrozn in zip(self.grid_mem, self.steps_mem, self.agents_xy_mem, self.agents_not_frozen_mem):
            contents = []
            data = self.return_grid(grid=g)
            contents.append(plt.imshow(data, cmap='inferno', extent=extent, animated=True))
            t1 = ax.text(x=0, y=0, s=f"step:{s}", fontsize="large", c="black")
            contents.append(t1)
            for na in range(np.shape(a_xy)[0]):
                if a_nfrozn[na, 0] == 1:
                    nax = ax.annotate(str(na), xy=(a_xy[na, 0, 1], a_xy[na, 0, 0]+1), xycoords='data', size=20)
                    contents.append(nax)
            a.append(contents)
            count += 1
            if count > 600:
        ...
        anim = animation.ArtistAnimation(fig, a, interval=1000, blit=True)
        anim.save("./3rdparty/hunterGame/replay/"+"hunter_game_test_"+datetime.now().strftime("%y-%m-%d_%H-%M-%S")+".gif", writer="imagemagick")
    ...
    def ifcatch_possible_step(self):
        # only used when agent=2 and stag = 1
        """
        return ture: for yes
        """
        assert self.n_agents==2 and self.n_stags==1, "ifcatch_possible_step method can only be used in agent=2 and stag = 1 situation"
        chk = 1. if np.sum(self.get_avail_actions(), axis=0)[5] == 2 else 0.
        return chk
